...
// 문제가 실기기에서 확인되었다. 우리 앱(WebView)이나 설치된 PWA는 실제로 safe-area
// 보정이 필요할 수 있으므로 제외한다.
//
// 주의(탐지 한계):
...
//   이 패턴을 갱신해야 한다.
const EVERYTIME_INAPP_UA_PATTERN = /everytime/i;

export const isEverytimeAndroidInAppBrowser = (): boolean => {
  if (typeof window === "undefined") return false;
  if (!isAndroid()) return false;
  if (isWebView()) return false;
  if (isPWA()) return false;

  return EVERYTIME_INAPP_UA_PATTERN.test(getUserAgent());
};
