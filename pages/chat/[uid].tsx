...
function Uid() {
  ...
  const { data: chatInfo } = useChatQuery(uid);
  ...
  useEffect(() => {
    ...
    setChats(
      [
        ...chatInfo,
        { content: "테스트입니다", createdAt: "2024-07-29", isMine: false, uid: "2222" },
      ].map((chat) => ({
  ...
  return (
    <>
      <Header title={`${opponent?.name}님과의 메세지`} rightPadding={4}>
        <Menu>
      ...
      <Slide>
        <Flex height="calc(100dvh - 118px)" bgColor="white" direction="column" p="16px">
          {chats.map((chat, idx) => {
            return (
              <Box key={idx} mb="16px">
                {chat.isMine ? (
