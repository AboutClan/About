...
function AlphabetChangeModal({
  ...
  const { mutate: requestAlphabet, isLoading } = useInteractionMutation("alphabet", "post", {
    onSuccess() {
    ...
    <ModalLayout title="알파벳 교환 신청" footerOptions={footerOptions} setIsModal={setIsModal}>
      ...
      <AlphabetContainer>
        {ABOUT.map((alphabet) => (
          ...
          >
            <AboutIcon size="sm" alphabet={alphabet} isActive={myAlphabets?.includes(alphabet)} />
        ...
        {ABOUT.map((alphabet) => (
          ...
          >
            <AboutIcon
              size="sm"
              alphabet={alphabet}
              isActive={opponentAlpabets?.includes(alphabet)}
            />
          </AlphabetBtn>
        ))}
      </AlphabetContainer>
