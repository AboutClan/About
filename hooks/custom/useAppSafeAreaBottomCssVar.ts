...
export function useAppSafeAreaBottomCssVar(): boolean {
  const [isBrokenSafeAreaAndroidInApp, setIsBrokenSafeAreaAndroidInApp] = useState(false);

  useIsomorphicLayoutEffect(() => {
    // 어바웃 RN WebView: 네이티브 컨테이너가 이미 처리하므로 0px.
    // 에브리타임/캠퍼스픽 Android 인앱 브라우저: env(safe-area-inset-bottom)이 실제로는
    ...
    const isBrokenSafeArea = !isApp() && isBrokenSafeAreaAndroidInAppBrowser();
    const value = isApp() || isBrokenSafeArea ? "0px" : "env(safe-area-inset-bottom, 0px)";

    document.documentElement.style.setProperty(APP_SAFE_AREA_BOTTOM_CSS_VAR, value);
    setIsBrokenSafeAreaAndroidInApp(isBrokenSafeArea);
  }, []);

  return isBrokenSafeAreaAndroidInApp;
}
