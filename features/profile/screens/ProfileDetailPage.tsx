...
import { useGroupsTitleQuery } from "@/features/group/hooks/queries";
...
function ProfileDetailPage({ user }: ProfileDetailPageProps) {
  ...
  const { data: count } = useGatherCountQuery(user?._id, { enabled: !!user?._id });

  const { data } = useGroupsTitleQuery(user?._id, {
    enabled: !!user?._id,
  });

  const { data: gatherData } = useGatherMyStatusQuery(0, user?._id, { enabled: !!user?._id });
      ...
      <Header title="">
        <HStack spacing={3}>
          {session?.user.id !== user?._id && (
            <>
              <ButtonWrapper onClick={handleChat}>
                <SendIcon />
              </ButtonWrapper>
              <ButtonWrapper
                onClick={() => {
